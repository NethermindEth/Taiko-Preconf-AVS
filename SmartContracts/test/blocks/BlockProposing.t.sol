...
contract BlockProposing is BlocksFixtures {
    ...
    function test_newBlockProposal_preconferCanProposeBlockInAdvanced_Case1() external {
        ...
        vm.assertEq(taikoL1.params(), bytes("Block Params"));
        vm.assertEq(taikoL1.txList(), bytes("Txn List"));
    }
    ...
    function test_newBlockProposal_preconferCanProposeBlockInAdvanced_Case2() external {
        ...
        // Verify that Taiko has received the block proposal
        vm.assertEq(taikoL1.params(), bytes("Block Params 2"));
        vm.assertEq(taikoL1.txList(), bytes("Txn List 2"));
    }
    ...
    function test_newBlockProposal_preconferCanProposeBlockAtDedicatedSlot() external {
        ...
        // Verify that Taiko has received the block proposal
        vm.assertEq(taikoL1.params(), bytes("Block Params 3"));
        vm.assertEq(taikoL1.txList(), bytes("Txn List 3"));
    }
    ...
    function test_newBlockProposal_updatesLookaheadForNextEpoch() external {
        ...
        vm.assertEq(lookaheadBuffer[3].timestamp, nextEpochStart + (9 * PreconfConstants.SECONDS_IN_SLOT));
        vm.assertEq(lookaheadBuffer[3].prevTimestamp, currentEpochStart + (22 * PreconfConstants.SECONDS_IN_SLOT));
        vm.assertEq(lookaheadBuffer[3].isFallback, false);

        // Check the second entry
        vm.assertEq(lookaheadBuffer[4].preconfer, addr_2);
        vm.assertEq(lookaheadBuffer[4].timestamp, nextEpochStart + (19 * PreconfConstants.SECONDS_IN_SLOT));
        vm.assertEq(lookaheadBuffer[4].prevTimestamp, nextEpochStart + (9 * PreconfConstants.SECONDS_IN_SLOT));
        vm.assertEq(lookaheadBuffer[4].isFallback, false);

        // Verify that the lookahead tail has been updated
        vm.assertEq(preconfTaskManager.getLookaheadTail(), 4);
