...
pub fn build_taiko_blob_sidecar(data: &[u8]) -> BlobTransactionSidecar {
    ...
    for raw_data_blob in chunks {
        ...
        let commitment =
            ethereum_consensus::crypto::kzg::blob_to_kzg_commitment(encoded_blob, kzg_settings)
                .unwrap();
        let proof = ethereum_consensus::crypto::kzg::compute_blob_kzg_proof(
            encoded_blob,
            &commitment,
            kzg_settings,
        )
...
mod tests {
    ...
    fn test_sidecar_builder_from_data() {
        ...
        let mut coder = TaikoBlobCoder::new();
        let encoded_blob: Blob = coder.encode_blob(&data);
